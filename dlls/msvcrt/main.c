...
static void qemu_init_dll(struct qemu_syscall *call)
{
    ...
    if (!msvcrt)
    {
        ...
        if (GetModuleHandleA(dll_name))
        {
            ...
            if (!my_errno)
            {
                /* Should not happen - _getptd() has allocated it... */
                WINE_FIXME("No msvcrt TLS data?\n");
            }
            ...
            else
            {
                DWORD idx = 0;
                BOOL found = FALSE;
                void *host_tls_data, *test_ptr;

                host_tls_data = CONTAINING_RECORD(my_errno, thread_data_t, thread_errno);

                do
                {
                    test_ptr = TlsGetValue(idx);
                    if (test_ptr == host_tls_data)
                    {
                        found = TRUE;
                        break;
                    }
        ...
        p_sscanf_s = (void *)GetProcAddress(msvcrt, "sscanf_s");
