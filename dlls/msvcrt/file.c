...
extern int CDECL call_sprintf( char *str, const char *format, void *func, int nb_args, int nb_onstack, const void *args );
__ASM_GLOBAL_FUNC( call_sprintf,
                   "stp x29, x30, [SP,#-16]!\n\t"           /* push FP & LR */
...
void qemu_sprintf(struct qemu_syscall *call)
{
    ...
    WINE_FIXME("(%lu floats/%lu args, onstack %i, format \"%s\"\n", c->argcount_float, c->argcount, onstack, (char *)QEMU_G2H(c->format));

    ret = call_sprintf(QEMU_G2H(c->str), QEMU_G2H(c->format), p_sprintf, c->argcount, onstack, c->args);
