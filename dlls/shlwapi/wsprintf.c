 ...
 *  Failure: -1.
 */
static INT my_wvnsprintfA( LPSTR buffer, INT maxlen, LPCSTR spec, va_list args )
{
    ...
    while (*spec && (maxlen > 1))
    {
        ...
        case WPR_CHAR:
            *p++ = argData.char_view;
            break;
        case WPR_STRING:
            my_memcpy( p, argData.lpcstr_view, len );
            p += len;
            break;
        case WPR_WSTRING:
            {
        ...
        case WPR_UNSIGNED:
            ...
            my_memcpy( p, number + sign, len - sign  );
            p += len - sign;
            break;
        case WPR_UNKNOWN:
            continue;
...
LWSTDAPI_(int) wvnsprintfA( LPSTR buffer, int maxlen, LPCSTR spec, va_list args )
{
 ...
 * See wvnsprintfA.
 */
static INT my_wvnsprintfW( LPWSTR buffer, INT maxlen, LPCWSTR spec, va_list args )
{
    WPRINTF_FORMAT format;
    LPWSTR p = buffer;
    UINT i, len, sign;
    ...
    while (*spec && (maxlen > 1))
    {
        ...
        case WPR_STRING:
            ...
            break;
        case WPR_WSTRING:
            if (len) my_memcpy( p, argData.lpcwstr_view, len * sizeof(WCHAR) );
            p += len;
            break;
        case WPR_HEXA:
            if ((format.flags & WPRINTF_PREFIX_HEX) && (maxlen > 3))
...
LWSTDAPI_(int) wvnsprintfW( LPWSTR buffer, int maxlen, LPCWSTR spec, va_list args )
{
...
LWSTDAPIV_(INT) wnsprintfA(LPSTR lpOut, int cchLimitIn, LPCSTR lpFmt, ...)
{
    ...
    res = my_wvnsprintfA( lpOut, cchLimitIn, lpFmt, valist );
    va_end( valist );
    return res;
}
...
LWSTDAPIV_(INT) wnsprintfW(LPWSTR lpOut, int cchLimitIn, LPCWSTR lpFmt, ...)
{
    ...
    va_start( valist, lpFmt );
    res = my_wvnsprintfW( lpOut, cchLimitIn, lpFmt, valist );
    va_end( valist );
    return res;
}
