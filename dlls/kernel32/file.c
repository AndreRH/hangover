...
WINBASEAPI BOOL WINAPI ReadFileEx(HANDLE hFile, LPVOID buffer, DWORD bytesToRead, LPOVERLAPPED overlapped, LPOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    ...
    call.bytesToRead = (uint64_t)bytesToRead;
...
void qemu_ReadFileEx(struct qemu_syscall *call)
{
    ...
    WINE_FIXME("Unverified!\n");
    c->super.iret = ReadFileEx(QEMU_G2H(c->hFile), QEMU_G2H(c->buffer), c->bytesToRead, QEMU_G2H(c->overlapped), QEMU_G2H(c->lpCompletionRoutine));
}
...
WINBASEAPI BOOL WINAPI ReadFile(HANDLE hFile, LPVOID buffer, DWORD bytesToRead, LPDWORD bytesRead, LPOVERLAPPED overlapped)
{
    ...
    call.buffer = (uint64_t)buffer;
    call.bytesToRead = (uint64_t)bytesToRead;
    call.bytesRead = (uint64_t)bytesRead;
    call.overlapped = (uint64_t)overlapped;
...
WINBASEAPI DWORD WINAPI GetFileSize(HANDLE hFile, LPDWORD filesizehigh)
{
    ...
    call.hFile = (uint64_t)hFile;
    call.filesizehigh = (uint64_t)filesizehigh;
...
WINBASEAPI BOOL WINAPI GetFileSizeEx(HANDLE hFile, PLARGE_INTEGER lpFileSize)
{
    ...
    call.hFile = (uint64_t)hFile;
    call.lpFileSize = (uint64_t)lpFileSize;
...
WINBASEAPI BOOL WINAPI GetFileTime(HANDLE hFile, FILETIME *lpCreationTime, FILETIME *lpLastAccessTime, FILETIME *lpLastWriteTime)
{
    ...
    call.lpLastAccessTime = (uint64_t)lpLastAccessTime;
    call.lpLastWriteTime = (uint64_t)lpLastWriteTime;
...
WINBASEAPI BOOL WINAPI SetFileTime(HANDLE hFile, const FILETIME *ctime, const FILETIME *atime, const FILETIME *mtime)
{
    ...
    call.hFile = (uint64_t)hFile;
    call.ctime = (uint64_t)ctime;
    call.atime = (uint64_t)atime;
    call.mtime = (uint64_t)mtime;
...
void qemu_CreateFileW(struct qemu_syscall *call)
{
    ...
    WINE_TRACE("\n");
    c->super.iret = (uint64_t)CreateFileW(QEMU_G2H(c->filename), c->access, c->sharing, QEMU_G2H(c->sa), c->creation, c->attributes, QEMU_G2H(c->template));
}
...
WINBASEAPI HANDLE WINAPI FindFirstFileExW(LPCWSTR filename, FINDEX_INFO_LEVELS level, LPVOID data, FINDEX_SEARCH_OPS search_op, LPVOID filter, DWORD flags)
{
    ...
    call.search_op = (uint64_t)search_op;
...
void qemu_FindFirstFileA(struct qemu_syscall *call)
{
    ...
    WINE_TRACE("\n");
    c->super.iret = (uint64_t)FindFirstFileA(QEMU_G2H(c->lpFileName), QEMU_G2H(c->lpFindData));
}
...
void qemu_FindFirstFileW(struct qemu_syscall *call)
{
    ...
    WINE_TRACE("\n");
    c->super.iret = (uint64_t)FindFirstFileW(QEMU_G2H(c->lpFileName), QEMU_G2H(c->lpFindData));
}
...
WINBASEAPI HANDLE WINAPI OpenVxDHandle(HANDLE hHandleRing3)
{
    ...
    call.hHandleRing3 = (uint64_t)hHandleRing3;
...
WINBASEAPI BOOL WINAPI K32EnumDeviceDrivers(void **image_base, DWORD cb, DWORD *needed)
{
    ...
    call.image_base = (uint64_t)image_base;
    call.cb = (uint64_t)cb;
