...
void qemu_d3d11_texture3d_GetDesc(struct qemu_syscall *call)
{
    ...
    /* D3D11_TEXTURE3D_DESC has the same size in 32 and 64 bit. */
    WINE_TRACE("\n");
    texture = QEMU_G2H(c->iface);
...
void qemu_d3d10_texture3d_GetDesc(struct qemu_syscall *call)
{
    ...
    /* D3D10_TEXTURE3D_DESC has the same size in 32 and 64 bit. */
    WINE_TRACE("\n");
    texture = QEMU_G2H(c->iface);
...
void __fastcall d3d11_texture_guest_destroy(struct qemu_d3d11_texture *texture)
{
...
static ULONG STDMETHODCALLTYPE d3d11_texture_priv_data_Release(IUnknown *iface)
{
    ...
    if (!refcount)
    {
        ...
        qemu_ops->qemu_execute(QEMU_G2H(d3d11_texture_guest_destroy), QEMU_H2G(texture));
