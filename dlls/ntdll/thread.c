...
void qemu_RtlFlsAlloc(struct qemu_syscall *call)
{
    ...
    WINE_TRACE("\n");
    if (c->callback)
        WINE_FIXME("FLS Release callback not implemented\n");
    c->super.iret = RtlFlsAlloc(NULL, QEMU_G2H(c->ret_index));
}
...
WINBASEAPI NTSTATUS WINAPI RtlFlsGetValue(ULONG index, void **data)
{
    ...
    /* FIXME: For performance reasons this really should avoid calling out
     * of the emulator. */
    qemu_syscall(&call.super);
...
void qemu_RtlFlsGetValue(struct qemu_syscall *call)
{
    ...
    c->super.iret = RtlFlsGetValue(c->index, &data);
