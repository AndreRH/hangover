...
{
    I386_CONTEXT *wow_context_arm = get_cpu_area(IMAGE_FILE_MACHINE_I386);
    NTSTATUS ret;

    ERR( "START %p %08lx\n", wow_context_arm, wow_context_arm->Eip );
    emu_run(wow_context_arm);
    ret = Wow64SystemServiceEx(wow_context_arm->Eax, (void*)wow_context_arm->Esp);
    ERR("RET IS %lX    %ld %lx\n", ret, wow_context_arm->Eax, wow_context_arm->Esp);
}
...
{
    static const UINT16 bopcode = 0x0f05;
